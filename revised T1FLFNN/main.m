...
for i=1:(length(time)-1)
    ...
    if i==1
        ...
        [rule,threshold_init(i),threshold_delete(i)] = T1_self_organize(rule,input);
    end

    [rule] = T1_adaption(rule,input);

    [O,rule]=T1_output(rule,input);

    u_T1(:,i)=M0*[O;zeros(3,1)];
    ...
    if norm(s(:,i))>tho
        ...
        u_sa(:,i)=norm(u_sm(:,i)+u_T1(:,i)+u_nc(:,i))/norm(s(:,i))*(1-0.65)*s(:,i);
    else
        ...
        u_sa(:,i)=norm(u_sm(:,i)+u_T1(:,i)+u_nc(:,i))/tho*(1-0.65)*s(:,i);
    end

    u(:,i)=u_sm(:,i)+u_T1(:,i)+u_nc(:,i)+u_sa(:,i);
